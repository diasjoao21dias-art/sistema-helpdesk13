from flask import Flask, render_template, request, redirect, url_for, session, send_file, abort, flash, g, jsonify
...
try:
    ...
    print("🔧 Melhorias de tipagem carregadas")
    ...
    # Funções de fallback com tipagem melhorada
...
def bootstrap():
    ...
    try:
        ...
        if sector_count == 0:
            default_sectors = [
                {'name': 'ti', 'display_name': 'T.I', 'description': 'Tecnologia da Informação'},
                {'name': 'manutencao', 'display_name': 'Manutenção', 'description': 'Manutenção geral'},
                {'name': 'ccih', 'display_name': 'CCIH / SESMT / Manutenção de Ar condicionado', 'description': 'Comissão de Controle de Infecção Hospitalar, SESMT e Manutenção de Ar condicionado'},
                {'name': 'telefonia', 'display_name': 'Telefonia e outros serviços', 'description': 'Telefonia e serviços diversos'}
            ]
            ...
            db.session.commit()
            print(f"✅ {len(default_sectors)} setores padrão criados")
        else:
            print(f"✅ {sector_count} setores já existem na base")
    except Exception as e:
        print(f"⚠️ Erro ao criar setores padrão: {e}")
        db.session.rollback()
...
# Integrar sistema de logging avançado
try:
    ...
    print("✅ Sistema de logging avançado carregado")
except ImportError as e:
    print(f"⚠️ Sistema de logging avançado não disponível: {e}")
    # Fallbacks
...
try:
    ...
    print("✅ Dashboard de métricas carregado")
except ImportError as e:
    print(f"⚠️ Dashboard de métricas não disponível: {e}")
...
try:
    ...
    print(f"⚠️ Endpoints REST não disponíveis: {e}")
...
def admin_metrics():
    ...
    try:
        system_health = metrics_dashboard.get_system_health() if metrics_dashboard else {'error': 'Dashboard indisponível'}
...
def admin_metrics_api(metric_type):
    """API para dados de métricas em JSON"""
    current_user = get_current_user()
    if not current_user or not current_user.is_admin:
...
# Monitorar performance de rotas críticas
@app.before_request
def before_request():
